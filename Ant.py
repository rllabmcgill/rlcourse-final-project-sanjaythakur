...
env = gym.make("Ant-v1")
env = wrappers.Monitor(env, './recordings/ant', force=True)
...
class Actor():
	def __init__(self, learning_rate=0.001, action_dof=2, scope="actor"):
		with tf.variable_scope(scope):
			self.state = tf.placeholder(dtype=tf.float32, shape=[1,111], name="state")
...
with tf.Session() as sess:
    ...
    for episode_iterator in range(NUM_EPISODES):
    	...
    	for episode_step in range(MAX_EPISODE_LENGTH):
    		...
    		if done:
    			print("Episode finished after {} timesteps".format(episode_step+1))
    			break
